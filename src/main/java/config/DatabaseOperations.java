...
public class DatabaseOperations {
    ...
    public static SimpleAnswer updateProfile(String userName, String email, String occupation, int phone, String accessToken) {
        RegisterAnswer rA = checkAccessToken(accessToken);
        if (rA.getSuccess()) {
            Neo4jTemplate template = main.createNeo4JTemplate();
            try {
                ...
                template.clear();
                template.purgeSession();
                return new SimpleAnswer(true, "values updated");
            } catch (NotFoundException nfe) {
                return new SimpleAnswer(false, "Invalid Accesstoken");
            }
        } else return new SimpleAnswer(false, rA.getReason());
    }
    ...
    public static SimpleAnswer uploadGoodybag(String creatorName, int creatorImage, String title, String status, String description,
        ...
        if (rA.getSuccess()) {
            ...
            try {
                ...
                return new SimpleAnswer(false, rA.getReason());
            }
        } else return new SimpleAnswer(false, rA.getReason());
